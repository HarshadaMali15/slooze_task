...
// GraphQL query to get individual carts by country (for Manager/Member)
const GET_COUNTRY_INDIVIDUAL_CARTS = gql`
  query GetCountryIndividualCarts {
    getCountryIndividualCarts {
      id
      country
      isShared
      userId
...
export default function CountryIndividualCartsPage() {
  const router = useRouter();
  const { data: cartsData, loading: cartsLoading, error: cartsError } = useQuery<{ getCountryIndividualCarts: Cart[] }>(GET_COUNTRY_INDIVIDUAL_CARTS);
  ...
  useEffect(() => {
    ...
    if (userData.role === 'ADMIN') {
      router.push('/cart/all-individual'); // Admin should go to all carts page
      return;
    }
  }, [router]);
  ...
  // Debug logging
  useEffect(() => {
    console.log('=== COUNTRY CARTS DEBUG ===');
    console.log('User Role:', userRole);
    console.log('User Country:', userCountry);
    console.log('Loading:', cartsLoading);
    console.log('Error:', cartsError);
    console.log('Carts Data:', cartsData);
    console.log('Carts Length:', cartsData?.getCountryIndividualCarts?.length || 0);
    
    // Debug each cart's user data
    cartsData?.getCountryIndividualCarts?.forEach((cart, index) => {
      ...
      console.log(`Cart ${index + 1} Country:`, cart.user?.country);
    });
    
    console.log('=== END DEBUG ===');
  }, [userRole, userCountry, cartsLoading, cartsError, cartsData]);
      ...
      <main className="min-h-screen bg-gradient-to-br from-red-500 via-orange-400 to-amber-400">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            <p className="text-white text-lg">Redirecting to Admin Carts...</p>
          </div>
        </div>
      </main>
    );
  ...
  console.log('Rendering carts:', cartsData?.getCountryIndividualCarts);
  console.log('Number of carts to render:', cartsData?.getCountryIndividualCarts?.length);

  return (
    <main className="min-h-screen bg-gradient-to-br from-green-500 via-emerald-400 to-teal-400">
      <header className="flex items-center justify-between px-6 py-4 bg-white/95 backdrop-blur shadow-sm border-b border-slate-200">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Country Individual Carts</h1>
          <p className="text-xs text-slate-500">View individual carts from users in your country ({userCountry})</p>
        </div>
      ...
      <div className="p-6">
        <div className="max-w-6xl mx-auto">
          <div className="mb-4 p-4 bg-white/95 rounded-lg border">
            <h3 className="text-sm font-semibold text-slate-900">Country Cart Info:</h3>
            <p className="text-xs text-slate-600">Your Country: {userCountry}</p>
            <p className="text-xs text-slate-600">Your Role: {userRole}</p>
            <p className="text-xs text-slate-600">Carts Found: {cartsData?.getCountryIndividualCarts?.length || 0}</p>
          ...
          <div className="grid gap-6">
            {cartsData?.getCountryIndividualCarts.map((cart) => {
              ...
              return (
                <div key={cart.id} className="bg-white/95 rounded-2xl shadow-lg p-6 border-2 border-green-200">
                  ...
                  {cart.items.length > 0 && (
                    <div className="border-t pt-4 mt-4">
                      <button
                        className="w-full px-4 py-2 rounded-full bg-green-500 text-white hover:bg-green-600 font-medium"
